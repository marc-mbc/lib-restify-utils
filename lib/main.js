...
module.exports.init = function(log) {
  ...
  return {
    ...
    /*
     *  Random errors. It's used to test the behaviour of a node micro service when crashes.
     */
    randomError: function () {
      var r = Math.random();
      if (r > 0.75) {
        log.debug('Random Error - Throw an Error');
        throw new Error('Dummy Exception');
      }
      else if (r > 0.5) {
        log.debug('Random Error - Causes an error');
        r = non_exist.non_exist + 1;
      }
      else {
        log.debug('Random Error - Normal behaviour');
      }
    }
  };
};
